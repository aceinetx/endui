	...
	case 'z':
		for (i = 0; i < handles->length; i++) {
			...
			if (window->parent == NULL) {
				if (*drag_window == NULL) {
					if (mouse->x >= window->x && (mouse->x < window->x + strlen(window->title)) && (mouse->y == window->y) && !(window->flags & EWH_NOMOVE) && !(window->flags & EWH_NOTITLE)) {
						*drag_window = window;
						break;
					}
				} else {
				...
				case EWH_BUTTON:
					if (mouse->x >= (window->parent->x + window->x) && (mouse->x < (window->parent->x + window->x + window->width)) && (mouse->y >= (window->parent->y + window->y)) && (mouse->y < (window->parent->y + window->y + window->height))) {
