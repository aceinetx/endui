...
void draw_windows(vec_void_t* handles, EWH* drag_window, endui_mouse* mouse) {
	...
	for (wi = 0; wi < handles->length; wi++) {
		...
		if (window->parent == NULL && window->hidden == false && window->virtual_desktop_id == *get_desktop_id_ptr()) {
			int end_y, end_x, x, y;
	...
	for (wi = 0; wi < handles->length; wi++) {
		...
		if (!window->hidden && window->virtual_desktop_id == *get_desktop_id_ptr()) {
			...
			case EWH_BUTTON:
				...
				text_x = window->x + window->parent->x + (window->width / 2) - (strlen(window->title) / 2);
				text_y = window->y + window->parent->y + (window->height / 2);
				mvprintw(text_y, text_x, "%s", window->title);
				break;
			case EWH_LABEL:
				mvprintw(window->parent->y + window->y, window->parent->x + window->x, "%s", window->title);
				break;
			case EWH_SEPARATOR:
				for (i = 0; i < window->width; i++) {
					mvprintw(window->parent->y + window->y, window->parent->x + window->x + i, "-");
				}
			}
		}
	}
